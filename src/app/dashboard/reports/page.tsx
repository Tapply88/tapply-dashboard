...
export default function ReportsPage() {
  ...
  const dailyTotal = useMemo(
    () => dailyRows.reduce((sum, r) => sum + r.total, 0),
    [dailyRows]
  );
  ...
  function exportDailyCsv() {
    ...
    const rows = dailyRows
      .map((r) => [r.date, r.count, r.total, r.cash, r.nonCash].join(','))
      .join('\n');
          ...
          {PLATFORMS.map((p) => (
            <div key={p.key} className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setCommissions((c) => ({ ...c, [p.field]: Number(e.target.value) || 0 }))
                }
